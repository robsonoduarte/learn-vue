    ...
    <div class="conteudo">
      <form class="painel">
        ...
        <Rotulo nome="Características do Problema">
					<span class="mr-4">
            <input type="checkbox" value="reproduzivel" v-model="characteristics"> Reproduzível
          </span>
          <span>
            <input type="checkbox" value="intermitente" v-model="characteristics"> Intermitente
          </span>
        </Rotulo>
        ...
        <Rotulo nome="Prioridade">
          <select v-model="priority">
            <option v-for="p in priorities"
                    :key="p.code"
                    :value="p.code"
                    :selected="p.code === 0"> {{ p.code }} - {{ p.name }}
            </option>
          </select>
        </Rotulo>
      ...
      <div class="painel">
        ...
        <Rotulo nome="Prioridade">
          <span>{{ priorities[priority].code }} - {{ priorities[priority].name }}</span>
        </Rotulo>
        <Rotulo nome="Primeira Reclamação?">
          {{ choice }}
        </Rotulo>
